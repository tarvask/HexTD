...
//                    this.PluginHost.LogDebug("SetupInstance config:" + config.SerializeToString());
        ...
        private void PostJsonRequest(string url, WebhooksRequest forwardRequest, HttpRequestCallback callback, ICallInfo info, bool callAsync = false)
        {
        ...
        private bool TryGetForwardResponse(IHttpResponse httpResponse, out WebhooksResponse response, out string errorMsg)
        {
            ...
            try
            {
                if (httpResponse.Status == HttpRequestQueueResult.Success)
                {
                    ...
                    var responseData = Encoding.UTF8.GetString(httpResponse.ResponseData).Trim(trimBomAndZeroWidth);
            ...
            {
                // since we prevalidate, we don't seam to get here
                errorMsg = $"{ex.Message}, data:{BitConverter.ToString(httpResponse.ResponseData, 0, Math.Min(1024, httpResponse.ResponseData.Length))}";
            }
