...
namespace Photon.Common.Misc
{
    public static class PublicIPAddressReader
    {
        ...
        private static IPAddress LookupInternalIpAddress()
        {
            ...
            throw new ConfigurationException("Could not determine internal IP address!");
        }
        ...
        private static IPAddress DoLookupPublicIpAddress(string lookupServiceUrl)
        {
            ...
            {
                log.Error(string.Format("Failed to lookup public ip address at {0}: Exception Msg:{1}", lookupServiceUrl, ex.Message), ex);
                throw;
            }
            finally
            {
