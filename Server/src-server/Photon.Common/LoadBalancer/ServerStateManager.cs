namespace Photon.Common.LoadBalancer
{
    ...
    public class ServerStateManager
    {
        ...
        public bool Start(string offlineFile)
        {
            ...
            {
                log.WarnFormat("Could not determine directory and fileName for server state file: {0}", this.appOfflineFile);
            }
            else
            {
        ...
        public void CheckAppOffline()
        {
            try
            {
                ...
                {
                    var old = this.workloadController.ServerState;

                    this.workloadController.ServerState = ServerState.Normal;

                    if (this.OnNewServerState != null)
                    {
                        this.OnNewServerState(old, ServerState.Normal, TimeSpan.Zero);
                    }
                ...
                {
                    log.InfoFormat("ServerState file is empty. Current Server state {0} is unchanged.", this.workloadController.ServerState);
                    return;
                }
                ...
                if (requestedState == (int)ServerState.Offline)
                {
                    if (log.IsInfoEnabled)
                    {
        ...
        private void ListenToPipe()
        {
            try
            {
                while (!this.stopListeningPipe)
                {
                    ...
                    {
                        log.Info($"Waiting for named pipe client connection. pipe={this.NamedPipeName}");
                        this.pipeServer.WaitForConnection();

                        log.Info($"Named pipe client connected. pipe={this.NamedPipeName}");

                        if (this.stopListeningPipe)
                        {
                            log.Info($"we are stopping server. break connection. pipe={this.NamedPipeName}");
                            break;
                        }
                        ...
                        try
                        {
                            ...
                            while (command != -1)
                            {
                                ...
                                if (log.IsDebugEnabled)
                                {
                                    log.Debug($"Received named pipe command: {command}, on pipe={this.NamedPipeName}");
                                }
        ...
        public void StartListenPipe()
        {
            if (log.IsInfoEnabled)
            {
                log.Info($"Watching for application state on named pipe '{this.NamedPipeName}'");
            }
