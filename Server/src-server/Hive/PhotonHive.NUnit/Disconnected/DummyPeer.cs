...
namespace Photon.Hive.Tests.Disconnected
{
    ...
    public class DummyPeer : IPhotonPeer
    {
        ...
        public void SetEncryption(byte[] encryptionSecret, byte[] hmacSecret, uint[] initialSeqNums)
        {
        }

        public void Trace(uint enable, object tag = null)
        {
            
        }

        public void GetTraceStatus(out uint traceOptions, out uint enabledOptions, out string tag)
        {
            throw new NotImplementedException();
        }
                ...
                case RtsMessageType.Event:
                    {
                        EventData eventData;
                        string errorMsg;
                        if (this.Protocol.TryParseEventData(data, out eventData, out errorMsg))
                        {
                            this.fiber.Enqueue(
                                () =>
                                    {
                ...
                case RtsMessageType.OperationResponse:
                    {
                        ...
                        string errorMsg;
                        if (this.Protocol.TryParseOperationResponse(data, out operationResponse, out errorMsg))
                        {
                            this.fiber.Enqueue(
                                () =>
                                    {
