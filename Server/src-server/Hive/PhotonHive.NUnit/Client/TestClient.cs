...
namespace Photon.Hive.Tests.Client
{
    ...
    public class TestClient : IDisposable, IPhotonPeerListener
    {
        ...
        {
            this.fiber.Enqueue(
                () => this.PhotonClient.SendOperation(operationRequest.OperationCode, operationRequest.Parameters,
        ...
        {
            var eventData = new EventData { Code = @event.Code, Parameters = (Dictionary<byte, object>)@event.Parameters };
            if (log.IsDebugEnabled)
            {
        ...
        public void OnOperationResponse(ExitGames.Client.Photon.OperationResponse operationResponse)
        {
            var response = new OperationResponse { 
                    ...
                    Parameters = (Dictionary<byte, object>)operationResponse.Parameters, 
