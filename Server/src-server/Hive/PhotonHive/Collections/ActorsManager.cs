...
namespace Photon.Hive.Collections
{
    public class ActorsManager : IEnumerable<Actor>
    {
                ...
                else
                {
                    stringBuilder
                        ...
                        .AppendFormat("nr:{0},usrId:'{1}',nick:'{2}',IsActive: {3}, peer: {4}",
                                    actor.ActorNr, actor.UserId, actor.Nickname, actor.IsActive, actor.Peer != null ? actor.Peer.ToString() : "LastPeerInfo: " + actor.LastPeerInfo)
                        .Append(")");
                }
            }
        ...
        public bool CheckMayOverrideSlots(string[] newExpectedUsersArray, int maxPlayers)
        {
            ...
            int expectedUsersNotJoinedYet = newExpectedUsersArray.Count(userId => this.GetActorByUserId(userId) == null);
        ...
        public bool TryAddExpectedUsers(HiveGame hiveGame, JoinGameRequest joinRequest)
        {
            ...
            var added = joinRequest.AddUsers.Where(userId => !this.IsExpectedUser(userId)).ToList();
        ...
        private bool CheckExcludeList(string userId, out string reason)
        {
            ...
            switch (info.Reason)
            {
                ...
                case RemoveActorReason.GlobalBanned:
                    reason = string.Format("User {0} is global banned", userId);
                    break;
                default:
