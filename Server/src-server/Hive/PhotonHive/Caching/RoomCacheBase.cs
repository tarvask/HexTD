...
namespace Photon.Hive.Caching
{
    ...
    public abstract class RoomCacheBase
    {
        ...
        /// <summary>used to synchronize access to the cache.</summary>
        ...
        public bool TryGetRoomWithoutReference(string roomId, out Room room)
        {
            lock (this.SyncRoot)
            {
                if (!this.RoomInstances.TryGetValue(roomId, out var roomInstance))
                {
                    room = null;
                    return false;
                }
        ...
        /// If the room with the specified id does not exist, a new room will be created.
        /// </summary>
        /// <param name="roomName">
        /// The room id.
        /// </param>
        ...
        public RoomReference GetRoomReference(string roomName, PeerBase ownerPeer, params object[] args)
        {
            lock (this.SyncRoot)
            {
                if (!this.RoomInstances.TryGetValue(roomName, out var roomInstance))
                {
        ...
        public int GetCount()
        {
            lock (this.SyncRoot)
            {
                return this.RoomInstances.Count;
            }
        }
        /// <summary>
        ...
        public virtual string GetDebugString(string roomName)
        {
            lock (this.SyncRoot)
            {
                if (!this.RoomInstances.TryGetValue(roomName, out var roomInstance))
                {
                    return $"RoomCache: No entry for room name {roomName}";
                }

                return $"RoomCache: RoomInstance entry found for room {roomName}: {roomInstance}";
            }
        }
        ...
        public void ReleaseRoomReference(RoomReference roomReference)
        {
            ...
            lock (this.SyncRoot)
            {
                if (!this.RoomInstances.TryGetValue(roomReference.Room.Name, out var roomInstance))
                {
                    return;
                }
                ...
                if (log.IsDebugEnabled)
                {
                    log.DebugFormat("Room has no references. Trying to remove it. room:{0}", roomReference.Room.Name);
                }
                // ask the room implementation if the room should be 
                // removed automatically from the cache
        ...
        /// <summary>
        /// Tries to remove a room instance from the room cache. 
        /// The room will only be removed if there are no references to the room instance left.
        /// </summary>
        /// <param name="room">
        /// The room to remove.
        /// </param>
        ...
        /// <summary>
        /// Must be implemented by inheritors to create new room instances.
        /// This method is called when a room reference is requested for a
        /// room that does not exist in the cache.
        /// </summary>
        /// <param name="roomId">
        /// The room id.
        /// </param>
            ...
            public RoomReference AddReference(PeerBase ownerPeer)
            {
                ...
                if (this.logQueue.Log.IsDebugEnabled)
                {
                    this.logQueue.Add(
                        new LogEntry(
                            "AddReference",
                            $"RoomName={this.Room.Name}, ReferenceCount={this.ReferenceCount}, OwnerPeer={ownerPeer}"));
                }
            ...
            public void ReleaseReference(RoomReference reference)
            {
                ...
                if (this.logQueue.Log.IsDebugEnabled)
                {
                    this.logQueue.Add(
                        new LogEntry(
                            "ReleaseReference",
                            $"RoomName={this.Room.Name}, ReferenceCount={this.ReferenceCount}, OwnerPeer={reference.OwnerPeer}"));
                }
            }
