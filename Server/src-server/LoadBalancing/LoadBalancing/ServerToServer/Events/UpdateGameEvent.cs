...
namespace Photon.LoadBalancing.ServerToServer.Events
{
    ...
    public class UpdateGameEvent : DataContract
    {
        ...
        public UpdateGameEvent(IRpcProtocol protocol, IEventData eventData)
            : this(protocol, eventData.Parameters)
        {
        }

        public UpdateGameEvent(IRpcProtocol protocol, Dictionary<byte, object> eventData)
            : base(protocol, eventData)
        {
        }
        ...
        [DataMember(Code = (byte)ServerParameterCode.Replication, IsOptional = true)]
        public byte? Replication { get; set; }
        ...
        public bool IsReplicationEvent
        {
            get { return this.Replication == ReplicationId.Replication || this.Replication == ReplicationId.ReplicationOfEmptyGame; }
        }

        public bool IsEmptyRoomReplication
        {
            get { return this.Replication == ReplicationId.ReplicationOfEmptyGame; }
        }
        #endregion
