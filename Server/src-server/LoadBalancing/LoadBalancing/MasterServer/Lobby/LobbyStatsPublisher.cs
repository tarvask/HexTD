...
namespace Photon.LoadBalancing.MasterServer.Lobby
{
    ...
    public class LobbyStatsPublisher : IDisposable
    {
        ...
        private void ExecuteGetStatsRequest(PeerBase peer, GetLobbyStatsRequest statsRequest, SendParameters sendParameter)
        {
            try
            {
                if (this.ValidateRequest(statsRequest, out OperationResponse errorresponse) == false)
                {
                ...
                {
                    log.Debug($"Executing GetStats request for peer:{peer}");
                }
                ...
                else
                {
                    ...
                    if (statsRequest.LobbyNames.Length > this.maxLobbyStatsCount)
                    {
        ...
        private void ExecuteSubscribe(PeerBase peer)
        {
            try
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.DebugFormat("Added subscription: peerId={0}", peer.ConnectionId);
                }
            }
            catch (Exception ex)
            {
        ...
        private void ExecuteUnsubscribe(PeerBase peer)
        {
            try
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.DebugFormat("Removed subscription: peerId={0}", peer.ConnectionId);
                }
            }
            catch (Exception ex)
            {
