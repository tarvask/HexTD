    ...
    public class GameTable
    {
        ...
        public void InsertGameState(string gameId, Dictionary<object, object> properties)
        {
            ...
            for (int i = 0; i < this.columnCount; i++)
            {
                ...
                {
                    this.insertCommand.Parameters[i + 1].Value = System.DBNull.Value;
                }
            }
        ...
        public void Update(string gameId, Dictionary<object, object> properties)
        {
            ...
            for (int i = 0; i < this.columnCount; i++)
            {
                ...
                {
                    this.updateCommand.Parameters[i + 1].Value = System.DBNull.Value;
                }
            }
        ...
        public string FindMatch(string query, int offset = 0)
        {
            var command = new SQLiteCommand();
            command.Connection = this.sqlConnection;
            command.CommandType = System.Data.CommandType.Text;
            command.CommandText = "SELECT Id FROM Game WHERE " + query + " LIMIT 1" + " OFFSET " + offset;
