...
namespace Photon.LoadBalancing.MasterServer.ChannelLobby
{
    ...
    public class GameChannel 
    {
        ...
        public void PublishGameChanges()
        {
            ...
            {
                var games = entry.Key == 0 ? this.GetChangedGames() : this.GetChangedGames(entry.Key);
        ...
        private Hashtable GetChangedGames()
        {
        ...
        private Hashtable GetChangedGames(int maxGameCount)
        {
        ...
        private Hashtable GetGameList(int maxCount)
        {
            if (maxCount <= 0)
            {
            ...
            var i = 0;
            foreach (var game in this.gameDict)
            {
        ...
        public class Subscription : GameSubscriptionBase
        {
            private readonly GameChannel GameChannel;
            ...
            protected override void Dispose(bool d)
            {
                ...
                this.GameChannel.gameChannelList.Lobby.EnqueueTask(this.SubscriptionCleanup);
            }

            private void SubscriptionCleanup()
            {
                HashSet<PeerBase> hashSet;
                if (this.GameChannel.subscriptions.TryGetValue(this.maxGamesCount, out hashSet) == false)
                {
                    log.WarnFormat("Failed to find game channel hashset for game count = {0}", this.maxGamesCount);
                    return;
                }
                ...
                if (hashSet.Count == 0)
                {
                    this.GameChannel.subscriptions.Remove(this.maxGamesCount);
                    if (log.IsDebugEnabled)
                    {
                        log.DebugFormat("Removed hashset for game count = {0}", this.maxGamesCount);
                    }
            ...
            {
                return this.GameChannel.GetGameList(this.maxGamesCount);
            }
        }
    }
}