...
namespace Photon.LoadBalancing.UnifiedClient
{
    public class UnifiedTestClient : UnifiedClientBase, IAuthSchemeClient
    {
        ...
        {
            Assert.IsTrue(this.TryWaitEvent(EventCode.ErrorInfo, timeout, out var eventData), "Did not get expected error event");
        ...
        {
            Assert.IsFalse(this.TryWaitEvent(EventCode.ErrorInfo, timeout, out var eventData),
                "Got unexpected error event: '{0}'", eventData?[ParameterCode.Info]);
        }
        ...
        {
            this.UserId = userId;
            var op = this.CreateAuthOnceRequest(userId);
        ...
        public OperationResponse Authenticate(string userId, Dictionary<byte, object> authParameter, short expectedErrorCode = 0)
        {
            ...
            var response = this.SendRequestAndWaitForResponse(op, expectedErrorCode, true);
            Assert.AreEqual(op.OperationCode, response.OperationCode);

            this.authenticationScheme.HandleAuthenticateResponse(this, (Dictionary<byte, object>)response.Parameters);
            ...
            int actorId, RoomOptions roomOptions, bool createIfNotExists, bool onGameServer, short errorCode = ErrorCode.Ok)
        {
            var joinRequest = new JoinGameRequest
            {
                ...
                JoinMode =
                    createIfNotExists
                        ? JoinModeConstants.JoinOnly
                        : JoinModeConstants.CreateIfNotExists
            };
        ...
        public JoinGameResponse JoinGame(JoinGameRequest joinRequest, short expectedResult = ErrorCode.Ok)
        {
            ...
            this.authenticationScheme.HandleAuthenticateResponse(this, (Dictionary<byte, object>)operationResponse.Parameters);
        ...
        {
            return this.JoinRandomGame(gameProperties, query, expectedResult, 255, new Dictionary<byte, object>(), lobbyName, lobbyType, expectedRoomNames);
        }
        ...
        public OperationResponse JoinRandomGame(Hashtable gameProperties, string query, short expectedResult, byte matchMakingType, Dictionary<byte, object> additionalParameters, string lobbyName = null, byte? lobbyType = null, params string[] expectedRoomNames)
        {
            ...
            if (!TryGetParameter(response, ParameterCode.RoomName, out string gameId))
            {
        ...
        {
            var operationRequest = CreateOperationRequest(OperationCode.GetGameList);
        ...
        public GetPropertiesResponse GetActorsProperties()
        {
            ...
            Assert.IsNotNull((Dictionary<byte, object>)response.Parameters);
        ...
        protected static void JoinRequestToDictionary(JoinGameRequest joinRequest, OperationRequest request)
        {
            ...
            if (joinRequest.JoinMode != JoinModeConstants.JoinOnly)
            {
        ...
        protected static CreateGameResponse GetCreateGameResponse(OperationResponse op)
        {
            var res = new CreateGameResponse
            {
                ...
                AuthenticationToken = GetParameter<object>(op, ParameterCode.Token, true),
            };
            return res;
        }
        ...
        protected static bool TryGetParameter<T>(OperationResponse response, byte parameterCode, out T value)
        {
            ...
            if (response.Parameters.TryGetValue(parameterCode, out var temp) == false)
            {
                return false;
            }
        ...
        {
            if (!TryGetParameter(response, parameterCode, out T value))
            {
