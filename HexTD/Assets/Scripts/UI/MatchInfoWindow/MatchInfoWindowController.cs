...
{
	public class MatchInfoWindowController : LoadableWindowController<MatchInfoWindowView>
	{
		private MatchRoundStartInfoWindowController _matchRoundStartInfoWindowController;
		private EnemyFieldViewWindowController _enemyFieldViewWindowController;

		public MatchInfoWindowController(IMessageReceiver messageReceiver)
		{
			messageReceiver.Receive<MainMenuController>().Subscribe(controller => Log.Debug(LogTag.None, 1));
		}

		protected override void DoInitialize()
		{
			_matchRoundStartInfoWindowController = CreateSubController<MatchRoundStartInfoWindowController>(
				View.MatchRoundStartInfoWindowView);
			_matchRoundStartInfoWindowController.Initialize();

			_enemyFieldViewWindowController = CreateSubController<EnemyFieldViewWindowController>(
				View.EnemyFieldViewWindowView);
			_enemyFieldViewWindowController.Initialize();
		}
