  ...
  public class MobileTouchCamera : MonoBehaviourWrapped {
    ...
    public void Awake() {
      ...
      Cam = GetComponent<Camera>();
            ...
            DebugText = GetComponent<DebugZoom>();
    ...
    public Vector3 GetIntersectionPointUnsafe(Ray ray) {
      ...
      return (ray.origin + ray.direction * distance);
        ...
        {
            //DebugText.DebugText.text = $"IsPinching = {IsPinching}, isTiltModeEvaluated = {isTiltModeEvaluated} isPinchTiltMode = {isPinchTiltMode} isPinchModeExclusive = {isPinchModeExclusive}";
            if (IsPinching == true)
            {
    ...
    private void UpdatePosition(float deltaTime) {
      ...
      if (IsDragging == false && IsPinching == false) {
        ...
        //Debug.Log($"magnitude is {overdragSpringVector.magnitude} below zero = {overdragSpringVector.magnitude > float.Epsilon}");
        if (overdragSpringVector.magnitude > float.Epsilon) {
          camPos += Time.deltaTime * overdragSpringVector * dragBackSpringFactor;
        }
      }

            var clampedPosition = GetClampToBoundaries(camPos);

            if (CheckVectorToNaN(clampedPosition) == false)
            {
                //Debug.Log($"Clamped position is NOT NaN");
                Transform.position = GetClampToBoundaries(camPos);
            }
            else
            {
                //Debug.Log($"Clamped position is NaN");
            }

    }

        private bool CheckVectorToNaN(Vector3 vector)
        {
    ...
    private void InputControllerOnInputClick(Vector3 clickPosition, bool isDoubleClick, bool isLongTap) {
      ...
      if(isLongTap == true) {
        return;
            ...
            Ray camRay = Cam.ScreenPointToRay(new Vector3(clickPosition.x, clickPosition.y, 10));
